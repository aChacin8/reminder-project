...
const EventsComponent = () => {
    ...
    const updateEvents = async (data) => {
        try {
                ...
                const response = await fetch(`${API_URL}/taskly/events/${data.id_event}`, {
                    method: 'PATCH',
                    headers: {
                        'Content-Type': 'application/json',
                        Authorization: `Bearer ${token}`
                            ...
                            <li key={event.id_events} className='mb-3' style={{ listStyle: 'none' }}>
                                <Card>
                                    ...
                                    <div className='eventCard__button'>
                                        <Button variant="primary" className='eventCard__buttonUpdate' onClick={()=> updateEvents(event.id_events)}>Editar</Button>
                                        <Button variant="danger" className='eventCard__buttonDelete'>Elimar</Button>
                                    </div>
                                </Card>
                            </li>
                        ))
